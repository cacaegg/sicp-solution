...
(define (next-r-pair ns last)
  (cond ((not (eq? (order-proc last) 
                   (order-proc (stream-car ns))))
         (next-r-pair (stream-cdr ns) (stream-car ns)))
        (else
          (cons-stream (order-proc last)
                       (next-r-pair (stream-cdr ns) (stream-car ns))))))

(define ramanujan-numbers
 (next-r-pair (stream-cdr ordered-pairs)
              (stream-car ordered-pairs)))
