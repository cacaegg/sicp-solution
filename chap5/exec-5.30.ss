...
(define (lookup-error code)
  (let ((error-codes
	 '((1 unbound-variable-error)
	   (2 unknown-expression-type-error)
	   (3 unknown-procedure-type-error))))
    (let ((error (assq code error-codes)))
      (if (not (eq? error #f))
	  (cadr error)
	  'unknown-error))))
...
(define (error-code error) (cdr error))
(define (make-error code) (cons 'error code))
...
(define prim-operations
  (list (list 'read read)
	...
	;; For error handling
	(list 'lookup-error lookup-error)
...
(define eceval
  (make-machine
   ...
   '(
    ...
    (assign val (op make-error) (const 2))
    (goto (label signal-error))
    unknown-procedure-type
    (restore continue)  ;; Clean up the stack from apply-dispatch
    (assign val (op make-error) (const 3))
    (goto (label signal-error))
    signal-error
    (assign val (op error-code) (reg val))
    (assign val (op lookup-error) (reg val))
    (perform (op user-print) (reg val))
    (goto (label read-eval-print-loop))
