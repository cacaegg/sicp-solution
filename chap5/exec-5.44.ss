...
(define (lambda-defines exp)
  (define (loop exp ret)
    (if (or (null? exp)
	    (not (definition? (car exp))))
	ret
	(loop (cdr exp) (cons (car exp) ret))))
  (loop exp '()))
(define (lambda-exps exp)
;;  (display (list 'lambda-exps exp)) (newline)
  (if (or (null? exp) (definition? (car exp)))
      (lambda-exps (cdr exp))
      exp))
(define (lambda-scan-out defines exps)
;;  (display (list 'lambda-scan-out defines exps)) (newline)
  (let ((def-vars (map definition-variable defines))
	(def-exps (map definition-value defines)))
    (let ((unassigned-vars
	   (map (lambda (v) ''*unassigned*)
		def-vars)))
      (let ((final-lambda
	     (append (list 'lambda
			   (if (null? def-vars) ''()
	...
	(if (null? def-vars)
	    final-lambda
	    (append (list final-lambda)
		    unassigned-vars))))))

;; (define l '(lambda (v) (define a 1) (define b 2) (* a b 3)))
;; (lambda-scan-out (lambda-defines (lambda-body l)) (lambda-exps (lambda-body l)))
...
(define (compile exp target linkage cenv)
  (display (list 'compiling exp target linkage)) (newline)
  (cond ((self-evaluating? exp)
	...
	((cond? exp)
	 (compile (cond->if exp) target linkage cenv))
        ((and (primitive-operation? exp)
	      (eq? 'not-found (find-variable (operator exp) cenv)))
	 (compile-primitive exp target linkage cenv))
	((application? exp)
...
(define (compile-lambda-body exp proc-entry cenv)
  (let ((formals (lambda-parameters exp))
	...
	(assign env
		...
		(reg argl)
		(reg env))))
     (compile-sequence (lambda-scan-out (lambda-defines body)
					(lambda-exps body))
		       'val 'return (extend-cenv formals cenv)))))
...
(define (spread-arguments operands cenv)
    (define (iter ops count)
      (let ((arg-reg
	     (string->symbol
	      (string-append (symbol->string 'arg)
			     (number->string count)))))
	(if (null? (cdr ops))
	    (compile (car ops) arg-reg 'next cenv)
	    (preserving '(arg-reg)
			(compile (car ops) arg-reg 'next cenv)
			(iter (cdr ops) (+ count 1))))))
    (iter operands 1))
(define (operator exp) (car exp))
(define (operands exp) (cdr exp))

(define (primitive-operation? exp)
  (display (list 'pri? (operator exp) (memq (operator exp) '(+ - * =)))) (newline)
  (not (eq? #f
	    (memq (operator exp) '(+ - * =)))))

(define (compile-primitive exp target linkage cenv)
  (if (not (= (length (operands exp)) 2))
      (error 'compile-primitive "Number of operands is not TWO" exp)
      (let ((linkage-code (compile-linkage linkage)))
	(preserving '()
