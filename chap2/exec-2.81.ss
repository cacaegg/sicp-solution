   ...
   (if (not (null? proc))
       ...
       (if (= (length args) 2)
           (let ((type1 (car type-tags))
                 ...
                 (a1 (car args))
                 (a2 (cadr args)))
            (if (not (eq? type1 type2))     ;; Do coercion only if types are not eq
...
(let ((x (make-scheme-number 2))
 ...
 (exp x y))
(let ((z1 (make-complex-from-real-imag 2 0))
      (z2 (make-complex-from-real-imag 3 0)))
 (exp z1 z2))
; a. Procedure will goes to an infinite loop due to is always coercion to complex, complex 
;    to another (apply-generic 'exp complex complex) look up.
; b. No, arguments with the same type should not be coercion. Since the purpose of coersion
;    is to converting types to find out right types in operation table. 
;    If it doesn't exist, we shoule simply raise error.
