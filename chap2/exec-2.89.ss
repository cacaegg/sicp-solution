...
(define (install-polynomial-package)
 ...
 (define (make-termlist term)
  (let ((ord (order term)))
   (if (> ord 0)
       (cons (coeff term) 
             (make-termlist (make-term (1- ord) 0)))
       (list (coeff term)))))
 ...
 (define (first-term term-list) 
  (list (1- (length term-list)) (car term-list)))
 ...
 (put 'make-termlist 'polynomial make-termlist)
...
(define (adjoin-term term tm-list)
 ((get 'adjoin-term 'polynomial) term tm-list))
(define (make-termlist term)
 ((get 'make-termlist 'polynomial) term))
...
(let ((t1 (make-term 1 2))
      ...
      (t3 (make-term 1 3))
      (t4 (make-term 10 2)))
 (let ((tls-1 (make-termlist t1))
       (tls-2 (make-termlist t3)))
  (let ((tls-1 (adjoin-term t2 tls-1))
        (tls-2 (adjoin-term t4 tls-2)))
   (display (list tls-1 tls-2))
   (newline))))

(let ((t1 (make-term 1 4))
      (t2 (make-term 4 5))
      (t3 (make-term 1 3))
      (t4 (make-term 10 2)))
 (let ((tls-1 (make-termlist t1))
