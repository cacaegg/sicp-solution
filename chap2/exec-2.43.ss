       ...
       (flatmap
        (lambda (rest-of-queens)
         (map (lambda (new-row)
              ...
              (enumerate-interval 1 board-size))) ; If put queen-cols here, board_size * (queen-cols (- k 1)) will
                                                  ; expand in to another board_size * board_size * (queen-cols (- k 2)
                                                  ; then until (- k n) equals to 0.
        (queen-cols (- k 1))))))                  ; This will make T^B (where B is board size)
 (queen-cols board-size))                         ; The original one only take B + k
                                                  ; So it's T^B time slower.
