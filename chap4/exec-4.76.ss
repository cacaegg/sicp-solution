...
(define (qeval query frame-stream)
  (display (list 'qeval query frame-stream)) (newline)
...
(define (conjoin conjuncts frame-stream)
  (display (list 'conjoin conjuncts frame-stream)) (newline)
...
(define (merge-frame-streams s1 s2)
  ; (display (list 'merge-frame-streams s1 s2)) (newline)
  (cond ((and (null? s1) (not (null? s2))) s2)
	((and (not (null? s1)) (null? s2)) s1)
	(else
	 (stream-flatmap
	  (lambda (f1)
	    ; (display (list 'flatmapping f1)) (newline)
	    (stream-filter
	     (lambda (f)
	       ; (display (list 'filtering f)) (newline)
	       (not (eq? f 'failed)))
	     (stream-map
	      (lambda (f2)
		; (display (list 'mappgin-frames f1 f2)) (newline)
		(merge-frames f1 f2))
	      s2)))
	  s1))))

(define (merge-frames fs1 fs2)
  ; (display (list 'merge-frames fs1 fs2)) (newline)
  (if (null? fs1)
      fs2
      (let ((var (caar fs1))
	    (val (cdar fs1)))
	(let ((extension (extend-if-consistent var val fs2)))
	  (if (eq? extension 'failed)
	      'failed
	      (merge-frames (cdr fs1) extension))))))
...
(define (negate operands frame-stream)
  (display (list 'negate operands frame-stream)) (newline)
  (stream-flatmap
   (lambda (frame)
...
(define (extend-if-possible var val frame)
  ; (display (list 'extend-if-possible var val frame)) (newline)
  (let ((binding (binding-in-frame var frame)))
    (cond (binding
           (unify-match
