;;; Make a history lookup & set in apply-a-rule function.
;;;
...
(define (qeval query frame-stream)
  ; (if (not (get-history (make-history-entry query)))
    (let ((qproc (get (type query) 'qeval)))
      ; (display (list 'query query))(newline)
      ; (display (list 'history-entry (make-history-entry query)))(newline)
      ; (put-history (make-history-entry query))
      (if qproc
  ...
  (let ((clean-rule (rename-variables-in rule)))
    (let ((unify-result
      ...
      (if (eq? unify-result 'failed)
          the-empty-stream
          (if (not (get-history history-entry))
              (begin 
                ; (display (list 'put-history history-entry))(newline)
                (put-history history-entry)
...
(define (make-table)
  (let ((local-table (list '*table*)))
    ...
    (define (insert! key-1 key-2 value)
      ...
      'ok)    
    (define reset!
      (lambda () (set! local-table (list '*table*))))
    (define (dispatch m)
...
(define (make-history-entry query)
  (if (null? query)
      '()
      (let ((token (car query)))
        (if (and (pair? token) 
                 (eq? '? (car token)))
            (cons '?var (make-history-entry (cdr query)))
            (cons token (make-history-entry (cdr query)))))))
...
(define (initialize-data-base rules-and-assertions)
  ...
  (let ((operation-table (make-table))
        (history-table (make-table)))
    (set! get (operation-table 'lookup-proc))
    (set! put (operation-table 'insert-proc!))
    (set! get-history 
      (lambda (query) 
        ((history-table 'lookup-proc) 'history query)))
    (set! put-history
      (lambda (query)
        ((history-table 'insert-proc!) 'history query #t)))
    (set! reset-history (history-table 'reset-proc!)))
  (put 'and 'qeval conjoin)
  (put 'or 'qeval disjoin)
  (put 'not 'qeval negate)
  (put 'lisp-value 'qeval lisp-value)
...
(rule (grandson ?grandpa ?grandson)
      (and (son ?grandpa ?papa)
           (son ?papa ?grandson)))

(rule (my-son ?groom ?what)
      (and (wife ?groom ?bride)
           (son ?bride ?what)))
