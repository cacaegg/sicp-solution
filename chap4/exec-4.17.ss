;; a. Due to let are expanded to lambda expression.
;;
;; b. It's due to both of solution prepare same context (not the same frames)
;;    before evaluating the body expression <e3>
;;
;; c. Change all the (define <var> <val>) to top scope of lambda
;;    And interpreter will not evaluate <val> until <var> is accessed
;;
