...
(rule (lives-near ?person-1 ?person-2)
      (and (address ?person-1 (?town . ?rest-1))
           ...
           (lisp-value ls->string-cmp<? ?person-1 ?person-2)))
...
(rule (subsitute ?subsitutor ?subsitutee)
      (and (job ?subsitutor ?stor-job)
           (job ?subsitutee ?stee-job)
           (not (same ?subsitutor ?subsitutee))
           (or (same ?stor-job ?stee-job)
               (can-do-job ?stor-job ?stee-job))))

(rule (bigshot ?people)
      (and (supervisor ?people ?super)
           (job ?people (?people-div . ?rest-people))
           (job ?super (?super-div . ?rest-super))
           (not (same ?people-div ?super-div))))

(rule (meeting-time ?person ?day-and-time)
      (and (meeting ?division ?day-and-time)
           (or (job ?person (?division . ?rest-div))
