...
(define (apply procedure arguments env)
  ; (display (list 'apply (car procedure) arguments (map car env)))(newline)
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (eval-sequence
           ...
           (let ((vars-vals 
                   (evaluate-compound-arguments (procedure-parameters procedure) 
                                                arguments env)))
            (extend-environment
              (map car vars-vals)
              (map cdr vars-vals)
...
(define (add-binding-to-frame! var val frame)
  ; (display (list 'add-binding var))(newline)
...
(define (lookup-variable-value var env)
  (define (env-loop env)
    (define (scan vars vals)
      ; (display (list 'scan-var var (if (pair? vars) (car vars))))(newline)
      (cond ((null? vars)
...
(define (evaluate-compound-arguments vars vals env)
  (map (lambda (var val)
         ; (display (list 'extend var val))(newline)
         (cond ((lazy-memo? var)
                (cons (car var) (delay-memo-it val env)))
               ((lazy? var)
                (cons (car var) (delay-it val env)))
               ((lazy-pair? var)
                (cons (cadr var) (caddr val)))
               ((primitive-procedure? val)
                (cons var val))
               (else
                (cons var (actual-value val env)))))
         vars vals))
...
(define (user-print object)
  (cond ((compound-procedure? object)
         (display (list 'compound-procedure
                        ...
                        '<procedure-env>)))
        ((lazy-pair? object)
         (display (list '<lazy-pair> (caadr object) '...)))
        (else (display object))))
